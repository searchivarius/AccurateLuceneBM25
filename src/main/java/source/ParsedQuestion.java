...
//        break;
    ...
    if (bestAnswId < 0) {
      System.out.println("The best answer is missing among answers for uri='"
          + mQuestUri + "'");
  ...
  private void reportDiffString(String s1, String s2, String Name) {
    if (s1 == null && s2 != null) {
      System.err.println(Name + ": the first is null, but the second is not");
    }
    if (s1 != null && s2 == null) {
      System.err.println(Name + ": the first is not null, but the second is");
    }    
    if (s1 != null && s2 != null && !s1.equals(s2)) {
      System.err.println(Name + ": different");
    }        
  }
  ...
  public boolean compare(Object obj, boolean bLogDiff) {
    if (this == obj)
      return true;
    if (obj == null) {
      System.err.println("The second object is null");
      return false;
    }
    if (getClass() != obj.getClass()) {
      System.err.println("Different object classes");
      return false;
    }
    ...
    if (mAnswers == null) {
      if (other.mAnswers != null) {
        System.err.println("One answer array is null, but the other is not");
        return false;
      }
    } else if (!mAnswers.equals(other.mAnswers)) {
      System.err.println("Answers are different");
      if (mAnswers.size() != other.mAnswers.size()) {
        System.err.println("Answer sizes are different");
      }
      else {
        for (int i = 0; i < mAnswers.size(); ++i) {
          if (!mAnswers.get(i).equals(other.mAnswers.get(i))) {
            System.err.println("Answers id= " + i + " are different");
          }
        }
      }
      return false;
    ...
    if (mBestAnswId != other.mBestAnswId) {
      System.err.println("Best answer IDs are different");
      return false;
    }
